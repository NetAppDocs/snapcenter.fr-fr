...
+ selon le libellé de snapshot que vous sélectionnez, ONTAP applique la règle de conservation de Snapshot secondaire qui correspond au libellé.

+ REMARQUE : si vous avez sélectionné *mettre à jour SnapMirror après avoir créé une copie Snapshot locale*, vous pouvez spécifier le libellé de stratégie secondaire. Toutefois, si vous avez sélectionné *mettre à jour SnapVault après la création d'une copie Snapshot locale*, vous devez spécifier l'étiquette de la stratégie secondaire.
