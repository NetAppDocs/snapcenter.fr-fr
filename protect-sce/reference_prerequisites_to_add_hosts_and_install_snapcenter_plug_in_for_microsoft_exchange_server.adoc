...
* Vous devez utiliser le Serveur Exchange de Microsoft pour les configurations autonomes et les groupes de disponibilité de bases de données. Consultez la link:https://imt.netapp.com/imt/#search["Outil de matrice d'interopérabilité (IMT)"^] pour connaître les versions prises en charge de Microsoft Exchange Server.
...
* Assurez-vous que les ports suivants ne sont pas bloqués par le pare-feu, sinon l'opération d'ajout d'hôte échouera. Pour résoudre ce problème, vous devez configurer la plage de ports dynamiques. Pour plus d'informations, consultez  https://docs.microsoft.com/en-us/troubleshoot/windows-server/networking/configure-rpc-dynamic-port-allocation-with-firewalls["Documentation Microsoft"^]. Reportez-vous à la link:https://imt.netapp.com/imt/#search["Outil de matrice d'interopérabilité (IMT)"^] pour connaître les versions prises en charge de Microsoft Serveur Exchange.
+
...
Packs logiciels requis
 a| 
* ASP.NET Core Runtime 8.0.24 (et tous les correctifs 8.0.x ultérieurs) Hosting Bundle
* PowerShell 7.4.13
* (Oracle Java ou OpenJDK) (ou Java 11 pour SnapCenter 6.2)
