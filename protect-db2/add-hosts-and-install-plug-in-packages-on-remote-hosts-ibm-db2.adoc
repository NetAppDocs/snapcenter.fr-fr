...
+
Si vous n'avez pas coché la case *Ignorer les prévérifications*, l'hôte est validé afin de vérifier s'il répond aux exigences d'installation du plug-in. L'espace disque, la RAM, la version de PowerShell, la version de .NET, l'emplacement (pour les plug-ins Windows), ainsi que Java 17, Java 11 (pour SnapCenter 6.2 uniquement) pour les plug-ins Windows et Linux sont validés par rapport aux exigences minimales. Si les exigences minimales ne sont pas satisfaites, des messages d'erreur ou d'avertissement appropriés s'affichent.
