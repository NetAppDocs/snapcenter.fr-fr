...
* Vous devez avoir installé Java 17 (pour SnapCenter 6.2.1 et versions ultérieures) ou
* Java 11 (pour SnapCenter 6.2 et versions antérieures) sur votre hôte Linux ou AIX.
+
** Java d'Oracle et d'OpenJDK est pris en charge pour Linux.
** IBM Java pour AIX. Vous pouvez le télécharger depuis https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["Téléchargements IBM Semeru RunTimes"^].
+

NOTE: Assurez-vous d'avoir installé uniquement l'édition certifiée de Java 17 (pour SnapCenter 6.2.1 et versions ultérieures) ou de Java 11 (pour SnapCenter 6.2 et versions antérieures) sur l'hôte Linux.
...
Packs logiciels requis
 a| 
* Java 17 (Oracle ou OpenJDK) pour SnapCenter 6.2.1 et versions ultérieures ou
* Java 11 pour SnapCenter 6.2 et versions antérieures



NOTE: Assurez-vous d'avoir installé uniquement l'édition certifiée de Java 17 pour SnapCenter 6.2.1 et versions ultérieures ou Java 11 pour SnapCenter 6.2 et versions antérieures sur l'hôte Linux.

Si vous avez mis à jour Java vers la dernière version, vous devez vous assurer que l'option JAVA_HOME située dans /var/opt/snapcenter/spl/etc/spl.properties est définie sur la version Java correcte et le chemin correct.
...
* Modifiez le  `/etc/ssh/sshd_config` fichier pour configurer les algorithmes de code d'authentification des messages : MACs hmac-sha2-256 et MACs hmac-sha2-512.
+
...
Packs logiciels requis
 a| 
* IBM Java 17 pour SnapCenter 6.2.1 et versions ultérieures ou
* IBM Java 11 pour SnapCenter 6.2 et versions antérieures
...
CAUTION: Bien que vous soyez autorisé à créer des identifiants après avoir déployé les hôtes et installé les plug-ins, la *bonne pratique* consiste à créer les identifiants après avoir ajouté les SVM, et avant de déployer les hôtes et d’installer les plug-ins.
...
NOTE: Si vous saisissez des informations incorrectes lors de la création d'un identifiant, un message d'erreur s'affiche. Vous devez sélectionner *Annuler*, puis réessayer.
...
. Sélectionnez image:../media/filter_icon.gif["icône de filtre"], puis sélectionnez le nom de l'hôte et le type de base de données pour filtrer les ressources.
+
Vous pouvez ensuite sélectionner image:../media/filter_icon.gif["icône de filtre"] pour fermer le volet de filtre.

. Sélectionnez la base de données, puis sélectionnez *Paramètres de la base de données* > *Configurer la base de données*.
...
+
Pour plus d'informations, consultez link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["Nœuds préférés dans la configuration RAC"^].
