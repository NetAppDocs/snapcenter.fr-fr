...
'''
Permalink: Protect-scsql/task_clone_backups_using_powershell_cmdlets_for_sql.html encadré: Mots-clés de la barre latérale: Résumé du clone: 'Le flux de travail du clone comprend la planification, l'exécution de l'opération de clonage et la surveillance de l'opération.' --- = clonage de sauvegardes à l'aide des applets de commande PowerShell :icons: Font :imagesdir: ../media/
