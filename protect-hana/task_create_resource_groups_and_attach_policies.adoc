...
* Pour ONTAP 9.12.1 et versions antérieures, les clones créés à partir des snapshots SnapLock Vault lors d'une restauration héritent de la date d'expiration SnapLock Vault. L'administrateur de stockage doit supprimer manuellement les clones après la date d'expiration SnapLock Vault.
...
. Sur la page Resources, sélectionnez *New Resource Group*.
...
Par exemple, si vous ajoutez des RH comme étiquette à plusieurs groupes de ressources, vous pourrez rechercher par la suite tous les groupes de ressources associés à la balise RH.
 ...
 a| 
Utilisez un format de nom personnalisé pour les snapshots.
 a| 
Cochez cette case et entrez un format de nom personnalisé que vous souhaitez utiliser pour le nom de l'instantané.

Par exemple, customtext_resourcegroup_policy_hostname ou resource group_hostname. Par défaut, un horodatage est ajouté au nom de l'instantané.
...
.. Sélectionnez la flèche *Scripts* et saisissez les commandes de pré et post-exécution pour les opérations de quiesce, snapshot et unquiesce. Vous pouvez également saisir les commandes de pré-exécution à exécuter avant de quitter en cas d'échec.
.. Sélectionnez la flèche *Configurations personnalisées* et saisissez les paires clé-valeur personnalisées requises pour toutes les opérations de protection des données utilisant cette ressource.
+
|===
...
Cette valeur doit être égale ou supérieure à NTAP_SNAPSHOT_RETENTIONS.
...
extension_fichier
 a| 
Spécifie une expression régulière qui correspond aux extensions de nom de fichier des fichiers journaux.

Pour les fichiers journaux se terminant par un certain nombre de chiffres, utilisez une expression régulière, telle que  `[0-9+]` ou (à partir de 6.0)  `\d+` (n'importe quelle longueur de chiffres d'au moins un caractère) comme valeur du paramètre, qui représente n'importe quel numéro de séquence de journal dans SAP HANA.
...
Vous devez utiliser ce paramètre si les journaux d'archivage se trouvent dans des sous-répertoires.

|===
+

NOTE: Les paires clé-valeur personnalisées sont prises en charge pour le SnapCenter Plug-in pour SAP HANA Database sur les systèmes Linux, mais pas pour les bases de données SAP HANA enregistrées auprès du SnapCenter Plug-in pour Microsoft Windows.
...
NOTE: Vous pouvez également créer une politique en sélectionnant *image:../media/add_policy_from_resourcegroup.gif["ajouter un groupe de ressources de formulaire de stratégie"]*.

+
Les stratégies sont répertoriées dans la section Configuration des planifications pour les stratégies sélectionnées.

.. Dans la colonne Configurer les planifications, sélectionnez *image:../media/add_policy_from_resourcegroup.gif["ajouter un groupe de ressources de formulaire de stratégie"]* pour la stratégie que vous souhaitez configurer.
