...
Le disque partagé n'est pas créé à l'aide du plug-in pour Windows. Vous devez créer, puis ajouter le LUN partagé à chaque machine virtuelle du cluster. Pour plus d'informations, reportez-vous à la section https://["Machines virtuelles de clusters sur des hôtes physiques"^].
