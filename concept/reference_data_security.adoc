...
+
Pour plus d'informations, voir https://["Configuration de la suite de cryptage SSL prise en charge"].
...
* IPSec est pris en charge avec SnapCenter sur ONTAP pour les machines hôtes Windows et Linux. https://["En savoir plus >>"].
...
* Prend en charge l'authentification de certificat d'autorité de certification entre le serveur SnapCenter et ONTAP. https://["En savoir plus >>"].
...
Vous pouvez également déployer le certificat d'autorité de certification pour le plug-in SnapCenter pour VMware vSphere. Pour plus d'informations, voir https://["Créer et importer des certificats"^].
