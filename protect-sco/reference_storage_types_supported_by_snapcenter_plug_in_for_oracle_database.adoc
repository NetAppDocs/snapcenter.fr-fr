...
Autres hyperviseurs
 a| 
Linux exécuté sous KVM, Xen, Hyper-V et les hyperviseurs hyperscaler est pris en charge lorsque le système d'exploitation invité dispose d'un accès TCP/IP direct aux ressources de stockage ONTAP requises en utilisant les méthodes suivantes :
...
NOTE: Les banques de données et les présentations de périphériques bruts ne sont pas prises en charge en dehors de VMware.
