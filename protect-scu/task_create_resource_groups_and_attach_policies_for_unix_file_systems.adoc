...
NOTE: Pour la cohérence du système de fichiers, des instantanés de groupe de cohérence sont pris pour les LUNs impliqués dans le groupe de volumes.
...
NOTE: Si vous avez ajouté différents systèmes de fichiers au groupe de ressources, alors tous les volumes de ces différents systèmes de fichiers dans le groupe de ressources sont placés dans un groupe de cohérence. Pour les sauvegardes cohérentes en cas de panne, un instantané de groupe de cohérence est pris par défaut même s'il n'y a qu'un seul système de fichiers ou volume.
